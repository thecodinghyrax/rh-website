...
from flask import Blueprint, url_for, request, render_template, redirect, url_for, flash
from ..models import db, Devotional, Calendar, User, Announcement, Application, UserMessages, Notes
from flask_login import login_required, current_user


manage = Blueprint('manage_bp', __name__,
    ...
    if request.method == 'POST':
        if request.form['approve'] == "True":
           ...
           Discord is our main voice and messaging system (https://discord.gg/4b5Bxh). \nIf you ever need anything don’t hesitate to ask any of our fine officers or friendly guild mates. \n\nWarmest welcome " + user.application.name
            ...
            from_user = request.form['from_user']
            from_user_image = request.form['from_user_image']
            message_body = request.form['reject_message']
            user_id = request.form['user_id']
            approve_message = UserMessages(from_user=from_user, from_user_image=from_user_image, message_body=message_body, user_id=user_id)
            try:
                db.session.commit()
            ...
            flash("The request.get('approve') was not = to 'True'. This should not happen. Screenshot this and send to Drew", "danger")
...
def manage_applications_status(status):
    ...
    if status == "pending":
        ...
        return redirect(url_for('manage_bp.admin'))
...
def manage_users(status):
    if current_user.rank > 5:
        ...
        return redirect(url_for('main_bp.index'))
    if status == "applicant":
        applications = Application.query.filter(Application.status == "Application recieved").filter(Application.user_id != None).order_by(Application.app_date.desc()).all()
        return render_template('manage_users.html', applications=applications, title=status)
    elif status == "member":
        applications = Application.query.filter(Application.status == "Approved").filter(Application.user_id != None).order_by(Application.app_date.desc()).all()
        users = User.query.options(db.load_only(User.id, User.rank)).all()
        return render_template('manage_users.html', applications=applications, users=users, rank_list=rank_list, title=status)
    elif status == "rejected":
        applications = Application.query.filter(Application.status == "Rejected").filter(Application.user_id != None).order_by(Application.app_date.desc()).all()
        return render_template('manage_users.html', applications=applications, title=status)
    elif status == "deleted":
...
def search():
    ...
    database = db_name_to_object[request.form['db']]
    results = database.query.filter(db.or_(database.title.like('%' + search + '%'), database.description.like('%' + search + '%'))).order_by(database.date.desc())
    data = results.count()
...
def delete():
    ...
    if request.form['search'] != "False":
        ...
        database = db_name_to_object[request.form['db']]
        results = database.query.filter(db.or_(database.title.like('%' + items_to_delete + '%'), database.description.like('%' + items_to_delete + '%')))
    ...
    if request.form['db'] == 'Announcement':
        ...
        messages_to_delete = UserMessages.query.filter_by(user_id=user_to_delete.id).all()
        try:

...
def guild_calendar():
    ...
    events = Calendar.query.filter(db.extract('month', Calendar.date) == month, db.extract('year', Calendar.date) == year).all()
...
def save_picture(form_picture, pic_to_delete):
    ...
    picture_path = path.join(root,'static','main', 'img', 'profile_pics', picture_fn)
    output_size = (125, 125)
    ...
    try:
        if pic_to_delete != None:
            picture_path_to_delete = path.join(root,'static','main', 'img', 'profile_pics', pic_to_delete)
            remove(picture_path_to_delete)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            from_user_image = '655c9f17511a4133.png'
            message_date = get_cst()
            message_body = 'You are now registered!\nWelcome to the home of the Renewed Hope guild. From here you can apply to join the guild, update your information or delete your account (see gear icon below).'
